...
INSTRUCTIONS = (
    ...
    "search_symbols, get_symbol_intel, "
    "get_company_risk_cn, quant_backtest, kimi_datasource, "
    "get_service_status, tv_scan, tv_analyze, egx_market, cex_market) — they auto-route across "
    ...
    "yahoo | fmp | alphavantage). get_symbol_intel bundles the per-symbol "
    "indicator queries — pass kind=options|short|analyst|dividends|earnings "
    "(options takes `expiration`, earnings takes `limit`). Use the futu_*-style "
    "tools (get_snapshot, get_kline, futu_get_option_chain, ...) for HK/CN/US "
    "depth from Futu OpenD. "
    "tv_scan/tv_analyze/egx_market are TradingView scanner containers selected by "
    "`action`; an unknown action returns the available list. cex_market quotes "
    "USDT-settled perpetuals on crypto exchanges (quote/kline/summary/funding/"
    "open_interest; a USDT PERPETUAL like CLUSDT.P, NOT the deliverable future "
    "CL=F / NYMEX:CL1! — ask action='symbols' first if unsure). quant_backtest runs "
